...
#DIR = "All_Data/MCMC_Summaries"
DIR = "First_Half/MCMC_Summaries"
...
# Output location
#OUT_NAME = 'All_Data_Comprehension_Summary.csv'
OUT_NAME = 'First_Half_Comprehension_Summary.csv'

# Use either the production or the comprehension mcdi
MCDI = 'comprehension-mcdi.csv'
...
colnames(Mean) = c('language', 'item.id', 'k_Mean', 'lambda_Mean', 'start_Mean')
HDIlow = dcast(data, language + item.id ~ X, value.var='HDIlow')
colnames(HDIlow) = c('language', 'item.id', 'k_HDIlow', 'lambda_HDIlow', 'start_HDIlow')
HDIhigh = dcast(data, language + item.id ~ X, value.var='HDIhigh')
colnames(HDIhigh) = c('language', 'item.id', 'k_HDIhigh', 'lambda_HDIhigh', 'start_HDIhigh')
